...
class ZachsButtons(object):
    ...
    def __init__(self, master):
        ...
        for i in range(self.griddim):
            self.buttons.append(Button(bFrame,width=self.buttonw, height=self.buttonh, bg=self.basecolor,command=partial(self.buttonColor,i,bFrame,topPadding)))
    ...
    def buttonColor(self, position,frame, padding):
        ...
        if self.colorfillvar == 0:
            ...
            buttonOK = Button(colorask, text = "OK", command= partial(self.getEntry,entry_1,position,frame,colorask, padding))
            ...
            try:
                temp = Button(frame, width=self.buttonw, height=self.buttonh, bg=self.fillbutton,command=partial(self.buttonColor, position, frame, padding))
            except:
    ...
    def getEntry(self, entry, position, frame, colorask, padding):
        ...
        if self.colorfillvar == 0:
            self.buttons[position].grid_forget()
            try:
                temp = Button(frame, width=self.buttonw, height=self.buttonh, bg=self.colorentry,command=partial(self.buttonColor, position, frame, padding))
            except:
                self.buttons[position] = Button(frame, width=self.buttonw, height=self.buttonh, bg='red',command=partial(self.buttonColor, position, frame, padding))
                self.buttons[position].grid(row=int(position / self.gridwidth), column=int(position % self.gridwidth))
                self.colors[position] = "red"
                ...
                return
            self.buttons[position] = Button(frame, width=self.buttonw, height=self.buttonh, bg=self.colorentry,command=partial(self.buttonColor, position, frame, padding))
            self.buttons[position].grid(row=int(position / self.gridwidth), column=int(position % self.gridwidth))
    ...
    def changeGrid(self, frame, padding):
        ...
        buttonOK = Button(gridask, text = "OK", command = partial(self.getgridEntries, entry_1, entry_2, gridask, frame, padding))
        label_1.grid(row=0, sticky=E)
        entry_1.grid(row=0, column=1)
    ...
    def getgridEntries(self, entry1, entry2, kill, frame, padding):
        ...
        self.buttons = []
        for i in range(self.griddim):
            self.buttons.append(Button(frame,width=self.buttonw, height=self.buttonh, bg=self.basecolor,command=partial(self.buttonColor,i,frame, padding)))
            self.buttons[i].grid(row=int((i/self.gridwidth)), column= int((i%self.gridwidth)))
            self.colors.append(self.basecolor)
        frame.pack()
        kill.destroy()
    ...
    def viewImage(self, size):
        ...
        for i in range(self.gridheight):
            for j in range(self.gridwidth):
                canvas.create_rectangle(j*size, i*size, j*size+size, i*size+size, fill=self.colors[i*self.gridwidth+j], width=0)
    ...
    def expFunc(self):
        ...
        file.write("uint16_t bitmap[] = {" + ','.join([self.getButtonColor(button) for button in self.buttons]) + "};\n\n")
        file.write("int idx = 0;\nfor (int i=0; i< " + str(self.gridheight) + "; i++){\n" + "\tfor (int j=0; j< " + str(self.gridwidth) +
    ...
    def importpic(self, frame, padding):
        ...
        buttonOK = Button(resizeask, text = "OK", command = partial(self.resizeandplace, entry_1, entry_2, entry_3, resizeask, frame, padding))
        label_1.grid(row=0, sticky=E)
        entry_1.grid(row=0, column=1)
        label_2.grid(row=1, column = 0)
        entry_2.grid(row=1, column=1)
    ...
    def resizeandplace(self, entry1, entry2, entry3, kill, frame, padding):
        ...
        for i in range(self.griddim):
            ...
            rgb = "#%0.6x" % rgb
            #print(rgb)
            self.buttons.append(Button(frame,width=self.buttonw, height=self.buttonh, bg=rgb,command=partial(self.buttonColor,i,frame, padding)))
